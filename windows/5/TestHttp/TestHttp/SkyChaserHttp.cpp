...
#pragma comment( lib, "libcurl140_a.lib")
...
CRITICAL_SECTION* CSkyChaserHttp::lockarray = NULL;
...
, m_isMulti(isMuliti)
{
	...
	//curl_easy_setopt(m_hCurl, CURLOPT_FORBID_REUSE, 1);

	//sc_setTimeOut(10* 1000);
}
...
CURLcode CSkyChaserHttp::sc_get( const char * sURL, const wchar_t * sFileName, bool bCaseRepHdr )
{
	if ( !m_hCurl )
...
void CSkyChaserHttp::sc_deleteHeader(const char * sHeader)
{
	curl_header_delete(&m_header, sHeader);
}

bool CSkyChaserHttp::curl_header_delete(struct curl_slist **csl_head, const char *data)
{
...
CURLcode CSkyChaserHttp::sc_getInfo(CURLINFO info, char*& url)
{   
    return curl_easy_getinfo(m_hCurl, CURLINFO_REDIRECT_URL, &url);
}
...
//取消重定向302
void CSkyChaserHttp::sc_cancelFollowaction()
{
    if (!m_hCurl)
        return;

    curl_easy_setopt(m_hCurl, CURLOPT_FOLLOWLOCATION, 0L);
}

//设置重定向302
void CSkyChaserHttp::sc_setFollowaction()
{
    if (!m_hCurl)
        return;
...
string CSkyChaserHttp::sc_convertCookie(string& strSrc)
{
    int n = 0;
    int nPos = 0;
    int nBeginPos = 0;
    ...
    for (int i = 0; i < 5; i++)
    {
        nPos = strSrc.find('\t', nPos);
        nPos++;
    }

    nBeginPos = nPos;

    while (nPos < strSrc.length())
    {
        nPos = strSrc.find('\t', nPos);
        if (nPos != std::string::npos)
        {
            //说明找到了
            strSrc.replace(nPos, 1, n % 2 == 0 ? "=" : ";");
            n++;
        }
    }

    string strRet;
    strRet = strSrc.substr(nBeginPos, strSrc.length() - nBeginPos);
    return strRet;
}

void CSkyChaserHttp::sc_replaceCookie( const char * sName, const char * sValue )
{
	if ( !m_hCurl )
		return;
    ...
    {
        sRet = cookies->data;
        nPos = sRet.find(sName, 0);
        
        if (nPos != std::string::npos)
        {
            int nPosStart = nPos + strlen(sName) + 1;
            nPos2 = sRet.find("\t", nPosStart);
            if (nPos2 == std::string::npos)
            {
                sRet.replace(nPosStart, sRet.length() - nPosStart, sValue);
            }
            else
            {
                sRet.replace(nPosStart, nPos2 - nPosStart, sValue);
            }
            memcpy(cookies->data, sRet.c_str(), sRet.length() + 1);
            curl_easy_setopt(m_hCurl, CURLOPT_COOKIELIST, cookies->data);
        }
...
void CSkyChaserHttp::sc_utf8ToUnicode( const string & sUtf8, wstring & sUnicode )
{
	...
	DWORD dwLen = MultiByteToWideChar(CP_UTF8, 0, sUtf8.c_str(), -1, NULL, 0);
	wchar_t * strUnicode = new wchar_t[dwLen];
	if (strUnicode == NULL)
		return;

	memset(strUnicode, 0, dwLen);
	MultiByteToWideChar(CP_UTF8, 0, sUtf8.c_str(), -1, strUnicode, dwLen);
	sUnicode = strUnicode;
	delete[] strUnicode;
}
...
void CSkyChaserHttp::sc_unicodeToUtf8( const wstring & sUnicode, string & sUtf8 )
{
	...
	DWORD dwUtf8Len = WideCharToMultiByte(CP_UTF8, 0, sUnicode.c_str(), -1, NULL, 0, NULL, NULL);
	char * strUtf8 = new char[dwUtf8Len];
	if (strUtf8 == NULL)
	{
		return;
	...
	memset(strUtf8, 0, dwUtf8Len);
	WideCharToMultiByte(CP_UTF8, 0, sUnicode.c_str(), -1, strUtf8, dwUtf8Len, NULL, NULL);
	sUtf8 = strUtf8;
...
void CSkyChaserHttp::lock_callback(int mode, int type, char *file, int line)
{
	(void)file;
	(void)line;
	if (mode & CRYPTO_LOCK) {
		EnterCriticalSection(&(lockarray[type]));
	}
	else {
		LeaveCriticalSection(&(lockarray[type]));
	}
}

unsigned long CSkyChaserHttp::thread_id(void)
{
	unsigned long ret;
...
void CSkyChaserHttp::init_locks(void)
{
	int i;

	lockarray = (CRITICAL_SECTION*)OPENSSL_malloc(CRYPTO_num_locks() *
		sizeof(CRITICAL_SECTION));

	for (i = 0; i<CRYPTO_num_locks(); i++) {
		InitializeCriticalSectionAndSpinCount(&lockarray[i], 0x00000400);
	}

	CRYPTO_set_id_callback((unsigned long(*)())thread_id);
	CRYPTO_set_locking_callback((void(__cdecl *)(int, int, const char *, int))lock_callback);
}

void CSkyChaserHttp::kill_locks(void)
{
	int i;

	CRYPTO_set_locking_callback(NULL);
	for (i = 0; i<CRYPTO_num_locks(); i++)
		DeleteCriticalSection(&(lockarray[i]));
