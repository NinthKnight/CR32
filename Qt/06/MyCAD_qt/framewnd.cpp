...
void FrameWnd::Serialise(QDataStream &ds, bool isWrite)
{
    if (isWrite){
        //写文件
        //首先写入个数
        ...
        for (auto shape : m_ShapeLst){
            QString strName = shape->metaObject()->className();
            ds << strName;
            shape->Serialise(ds);
        }
        ...
        for (int i = nLstSize; i > 0; i--){
            QString strName;

            ds >> strName;
            //首先拿对象的id
            int nId = QMetaType::type(strName.toStdString().c_str());

            if(nId != QMetaType::UnknownType){
             //根据ID创建对象
             IShape* pShape = (IShape*)QMetaType::create(nId);

             pShape->Serialise(ds, false);

             m_ShapeLst.push_back(QSharedPointer<IShape>(pShape));
